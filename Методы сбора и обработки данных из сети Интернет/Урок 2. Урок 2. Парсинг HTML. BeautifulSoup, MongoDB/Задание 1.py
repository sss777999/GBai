...
count = 1
while True:
    params = {'text': 'Artificial intelligence',  # то, что тут + не нужен кардинально меняет ситуацию!
              'fromSearch': 'true',
              'area': '1',
              'st': 'searchVacancy',
    ...
    for i in vacansies_list:
        if i['class'] == ['vacancy-serp-item']:
            ...
            try:
                ...
                salary = 'None'
            vac.append([count, name, salary, url, site])  # count оставлю на всякий случай
            count += 1
    page += 1
...
for i in vac:
    if i[2] != 'None':
        ...
        if '-' in i[2]:
            ...
            i[2] = i[2].split(' ')
# делаем датафрейм
data = pd.DataFrame(vac, columns=['index', 'name', 'salary', 'uri', 'site'])
for i in vac:
    if i[2] != 'None':
