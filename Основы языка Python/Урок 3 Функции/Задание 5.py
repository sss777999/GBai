...
def sumstr(string):
    global counter
    while string != '':
        if len(string) >= 3 and string[-1] == '/':              # специальный символ введен после нескольких чисел
            string.pop()
            string = [int(i) for i in string]
            counter += sum(string)
            print(f'Sum of integers after "/" is {counter}')
            break
        if string and string[0] == '/':                         # специальный символ, выполнение программы завершается
            break
        else:
            ...
            try:
                ...
                a = False
                while a is False:
                    string = input(f'Input ONLY integers, you entered wrong data {string}: ').split()
                    if len(string) >= 3 and string[-1] == '/':  # специальный символ введен после нескольких чисел
                        string.pop()
                        string = [int(i) for i in string]
                        print(f'Sum of integers after "/" is {counter}')
        ...
        string = input('Input two integers with space ro "/" for exit: ').split()
